...
<#macro userProfileFormFields>
	<#assign currentGroup="">
	<#-- Registration order: first/last → email → username (passwords injected in register.ftl) -->
	<#assign pvOrdered = ["firstName", "lastName", "email", "username"]>
	...
	<#list pvOrdered as pvOname>
		<#list profile.attributes as attribute>
			<#if attribute.name == pvOname>
				...
				<#if currentGroup != "">
					...
					>
						...
						<#if groupDisplayHeader != "">
							<#assign groupHeaderText=advancedMsg(group.displayHeader)!group>
						<#else>
						...
						<#if groupDisplayDescription != "">
							<#assign groupDescriptionText=advancedMsg(group.displayDescription)!"">
	...
	<#list profile.attributes as attribute>
		<#if attribute.name=='locale' && realm.internationalizationEnabled && locale.currentLanguageTag?has_content>
		<#elseif pvOrdered?seq_contains(attribute.name)>
		<#else>
