...
| `edge` / `edge-tls` | Stateless | Public entry point. Only service publishing host ports. |
...
|----------|-----------------|---------|
| Internet / customer network to `edge` | Browser traffic, Runtime API calls, Keycloak browser flows | Customer TLS/ingress policy, `BASE_URL`, `CORS_ORIGINS`, `KC_PUBLIC_HOSTNAME`. |
...
- Terminate TLS at your ingress or use the `direct_tls` profile.
